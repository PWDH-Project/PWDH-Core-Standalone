...
BASE_SCRIPTS = [
    ...
    'interface_pwdh_cli.py --legacy-wallet',
    'interface_pwdh_cli.py --descriptors',
...
def main():
    ...
    enable_pwdhd = config["components"].getboolean("ENABLE_PWDHD")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, skipunit=False):
    ...
    # Warn if pwdhd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "pwdhd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a pwdhd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/pwdh/pwdh/pull/27561.
...
class RPCCoverage():
    ...
    commands per `pwdh-cli help` (`rpc_interface.txt`).
