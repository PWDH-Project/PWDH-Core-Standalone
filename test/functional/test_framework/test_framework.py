...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    """Base class for a pwdh test script.

    Individual pwdh test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave pwdhds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop pwdhds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use pwdh-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "pwdhd": ("pwdhd", "PWDHD"),
            "pwdh-cli": ("pwdhcli", "PWDHCLI"),
            "pwdh-util": ("pwdhutil", "PWDHUTIL"),
            "pwdh-wallet": ("pwdhwallet", "PWDHWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: pwdhds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'pwdhd', self.options.pwdhd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'pwdh-cli', self.options.pwdhcli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a pwdhd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple pwdhd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the pwdhd
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as pwdhd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    pwdhd=self.options.pwdhd,
                    pwdh_cli=self.options.pwdhcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in pwdh.conf
    ...
    def skip_if_no_pwdhd_tracepoints(self):
        """Skip the running test if pwdhd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("pwdhd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_pwdhd_zmq(self):
        """Skip the running test if pwdhd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("pwdhd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if pwdh-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("pwdh-wallet has not been compiled")

    def skip_if_no_pwdh_util(self):
        """Skip the running test if pwdh-util has not been compiled."""
        if not self.is_pwdh_util_compiled():
            raise SkipTest("pwdh-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if pwdh-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("pwdh-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether pwdh-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether pwdh-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_pwdh_util_compiled(self):
        """Checks whether pwdh-util was compiled."""
        return self.config["components"].getboolean("ENABLE_PWDH_UTIL")
