This directory contains integration tests that test pwdhd and its
...
pwdhd and pwdh-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (pwdh-util, pwdh-tx, ...).
...
The P2P and RPC ports used by the pwdhd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another pwdhd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its pwdhd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other pwdhd processes are running.

On linux, the test framework will warn if there is another
pwdhd process running when the tests are started.

If there are zombie pwdhd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
pwdhd processes running on the system, so should not be used if any non-test
pwdhd processes are being run.**
...
sure pwdhd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and pwdhd `debug.log`s will all be dumped
...
`test_framework.log` and pwdhd `debug.log`s can be combined into a single
...
call methods that interact with the pwdhd nodes-under-test.

If further introspection of the pwdhd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/pwdhd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/pwdhd <pid>
```
