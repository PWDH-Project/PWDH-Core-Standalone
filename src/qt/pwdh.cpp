...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<PWDHUnit>("PWDHUnit");
#else
    qRegisterMetaType<PWDHUnit>("PWDHUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. pwdh_de.qm (shortcut "de" needs to be defined in pwdh.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. pwdh_de_DE.qm (shortcut "de_DE" needs to be defined in pwdh.qrc)
...
static const char* qt_argv = "pwdh-qt";

PWDHApplication::PWDHApplication()
...
void PWDHApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the PWDHApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", PWDHGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void PWDHApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool PWDHApplication::createOptionsModel(bool resetSettings)
{
...
void PWDHApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new PWDHGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &PWDHGUI::quitRequested, this, &PWDHApplication::requestShutdown);
...
void PWDHApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void PWDHApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool PWDHApplication::baseInitialize()
{
    return node().baseInitialize();
}

void PWDHApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &PWDHApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &PWDHApplication::handleRunawayException);
    connect(this, &PWDHApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &PWDHApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void PWDHApplication::parameterSetup()
{
...
void PWDHApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void PWDHApplication::requestInitialize()
{
...
void PWDHApplication::requestShutdown()
{
...
void PWDHApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // pwdh: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PWDHGUI::handlePaymentRequest);
            connect(window, &PWDHGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void PWDHApplication::handleRunawayException(const QString &message)
{
...
void PWDHApplication::handleNonFatalException(const QString& message)
{
...
WId PWDHApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool PWDHApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", PWDHGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(pwdh);
    Q_INIT_RESOURCE(pwdh_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(PWDH_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see pwdh-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see pwdh-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse pwdh.conf, determine network, switch to network specific
    ...
    // pwdh: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
