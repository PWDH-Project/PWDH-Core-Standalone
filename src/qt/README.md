This directory contains the source code for the PWDH Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the PWDH UI.
...
- ClientModel is used by the main application `pwdhgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a pwdh:... link from a browser).
...
* UI elements like PWDHAmountField, which inherit from QWidget.
* `pwdhstrings.cpp`: automatically generated
* `pwdhunits.(h/cpp)`: BC2 / mBC2 / etc. handling
* `callback.h`
...
**Note:** Do not change `local/pwdh_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `pwdh-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `pwdh-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `pwdh-qt`
9. You're all set! Start developing, building, and debugging the PWDH Core GUI
