...
#include <qt/pwdhaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PWDH address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new PWDHAddressEntryValidator(parent));
    widget->setCheckValidator(new PWDHAddressCheckValidator(parent));
}
...
bool parsePWDHURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no pwdh: URI
    if(!uri.isValid() || uri.scheme() != QString("pwdh"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!PWDHUnits::parse(PWDHUnit::BC2, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parsePWDHURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsePWDHURI(uriInstance, out);
}

QString formatPWDHURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("pwdh:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PWDHUnits::format(PWDHUnit::BC2, info.amount, false, PWDHUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openPWDHConf()
{
    ...
    /* Open pwdh.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PWDH.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PWDH (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("PWDH (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "pwdh.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("pwdh-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a pwdh.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=PWDH\n";
        else
            optionFile << strprintf("Name=PWDH (%s)\n", ChainTypeToString(chain));
