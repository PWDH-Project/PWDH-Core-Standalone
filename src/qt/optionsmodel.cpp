...
/** Call node.updateRwSetting() with PWDH 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because PWDH 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/pwdh/pwdh/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because pwdh 22.x will not try to read these.
...
//! Convert enabled/size values to pwdh -prune setting.
...
//! Get pruning enabled value to show in GUI from pwdh -prune setting.
...
//! Get pruning size value to show in GUI from pwdh -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayPWDHUnit")) {
        settings.setValue("DisplayPWDHUnit", QVariant::fromValue(PWDHUnit::BC2));
    }
    QVariant unit = settings.value("DisplayPWDHUnit");
    if (unit.canConvert<PWDHUnit>()) {
        m_display_pwdh_unit = unit.value<PWDHUnit>();
    } else {
        m_display_pwdh_unit = PWDHUnit::BC2;
        settings.setValue("DisplayPWDHUnit", QVariant::fromValue(m_display_pwdh_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if pwdh.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_pwdh_unit);
...
{
    if (new_unit.isNull() || new_unit.value<PWDHUnit>() == m_display_pwdh_unit) return;
    m_display_pwdh_unit = new_unit.value<PWDHUnit>();
    QSettings settings;
    settings.setValue("DisplayPWDHUnit", QVariant::fromValue(m_display_pwdh_unit));
    Q_EMIT displayUnitChanged(m_display_pwdh_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/pwdh/pwdh/pull/8273
    ...
    // (https://github.com/pwdh-core/gui/issues/567).
