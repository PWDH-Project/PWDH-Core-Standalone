...
const std::string PWDHGUI::DEFAULT_UIPLATFORM =
...
PWDHGUI::PWDHGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &PWDHGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &PWDHGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &PWDHGUI::showModalOverlay);
...
void PWDHGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a PWDH address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and pwdh: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &PWDHGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &PWDHGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &PWDHGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your PWDH addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified PWDH addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed PWDH Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed PWDH Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a pwdh: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible PWDH command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &PWDHGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &PWDHGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &PWDHGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &PWDHGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &PWDHGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &PWDHGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &PWDHGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &PWDHGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &PWDHGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &PWDHGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &PWDHGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &PWDHGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &PWDHGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &PWDHGUI::showDebugWindow);
}

void PWDHGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void PWDHGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &PWDHGUI::setCurrentWalletBySelectorIndex);
...
void PWDHGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &PWDHGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &PWDHGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &PWDHGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &PWDHGUI::showProgress);
...
#ifdef ENABLE_WALLET
void PWDHGUI::enableHistoryAction(bool privacy)
{
...
void PWDHGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &PWDHGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &PWDHGUI::removeWallet);
...
WalletController* PWDHGUI::getWalletController()
{
    return m_wallet_controller;
}

void PWDHGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &PWDHGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &PWDHGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &PWDHGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &PWDHGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &PWDHGUI::incomingTransaction);
    connect(this, &PWDHGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void PWDHGUI::removeWallet(WalletModel* walletModel)
{
...
void PWDHGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void PWDHGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void PWDHGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PWDHGUI::setWalletActionsEnabled(bool enabled)
{
...
void PWDHGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &PWDHGUI::toggleHidden);
...
void PWDHGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void PWDHGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void PWDHGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void PWDHGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void PWDHGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void PWDHGUI::openClicked()
{
...
void PWDHGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PWDHGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void PWDHGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void PWDHGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PWDHGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PWDHGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void PWDHGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void PWDHGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to PWDH network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void PWDHGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void PWDHGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void PWDHGUI::updateHeadersSyncProgressLabel()
{
...
void PWDHGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void PWDHGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &PWDHGUI::quitRequested);
...
void PWDHGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void PWDHGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &PWDHGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void PWDHGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void PWDHGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &PWDHGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &PWDHGUI::show);
                e->ignore();
            }
        }
    }
...
void PWDHGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PWDHGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PWDHGUI::incomingTransaction(const QString& date, PWDHUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(PWDHUnits::formatWithUnit(unit, amount, true));
...
void PWDHGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PWDHGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PWDHGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PWDHGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PWDHGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void PWDHGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void PWDHGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void PWDHGUI::updateWindowTitle()
{
...
void PWDHGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PWDHGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PWDHGUI::detectShutdown()
{
...
void PWDHGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void PWDHGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(PWDHGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = PWDHGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void PWDHGUI::subscribeToCoreSignals()
{
...
void PWDHGUI::unsubscribeFromCoreSignals()
{
...
bool PWDHGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<PWDHUnit> units = PWDHUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const PWDHUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, PWDHUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const PWDHUnit u : PWDHUnits::availableUnits()) {
        menu->addAction(PWDHUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(PWDHUnit newUnits)
{
    setText(PWDHUnits::longName(newUnits));
}
