...
static void TestBlockSubsidySchedule(const Consensus::Params& consensusParams)
{
    constexpr CAmount kInitialSubsidy = 35 * COIN;
    constexpr int64_t kHalvingsUntilFloor = 6;
    constexpr int64_t kStableFloorYears = 15;
    const int64_t yearly_interval = consensusParams.nSubsidyHalvingInterval;

    BOOST_CHECK(yearly_interval > 0);

    for (int64_t era = 0; era <= kHalvingsUntilFloor; ++era) {
        BOOST_CHECK_EQUAL(GetBlockSubsidy(era * yearly_interval, consensusParams), kInitialSubsidy >> era);
    }

    const CAmount floor_subsidy = kInitialSubsidy >> kHalvingsUntilFloor;
    BOOST_CHECK_EQUAL(GetBlockSubsidy((kHalvingsUntilFloor + 1) * yearly_interval, consensusParams), floor_subsidy);
    BOOST_CHECK_EQUAL(GetBlockSubsidy((kHalvingsUntilFloor + kStableFloorYears - 1) * yearly_interval, consensusParams), floor_subsidy);
    BOOST_CHECK_EQUAL(GetBlockSubsidy((kHalvingsUntilFloor + kStableFloorYears) * yearly_interval, consensusParams), 0);
}

BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    TestBlockSubsidySchedule(chainParams->GetConsensus());
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    constexpr int64_t kHalvingsUntilFloor = 6;
    constexpr int64_t kStableFloorYears = 15;
    const int64_t yearly_interval = chainParams->GetConsensus().nSubsidyHalvingInterval;

    CAmount total_subsidy = 0;
    for (int64_t era = 0; era <= (kHalvingsUntilFloor + kStableFloorYears - 1); ++era) {
        const CAmount era_subsidy = era <= kHalvingsUntilFloor ? (kInitialSubsidy >> era) : (kInitialSubsidy >> kHalvingsUntilFloor);
        BOOST_CHECK(era_subsidy <= kInitialSubsidy);
        total_subsidy += era_subsidy * yearly_interval;
        BOOST_CHECK(MoneyRange(total_subsidy));
    }
    BOOST_CHECK_EQUAL(total_subsidy, CAmount{1621147500000000});
}
