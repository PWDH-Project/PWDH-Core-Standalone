...
const char * const PWDH_CONF_FILENAME = "pwdh.conf";
const char * const PWDH_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //pwdh-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", PWDH_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PWDH";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/PWDH";
#else
    // Unix-like
    return pathRet / ".pwdh";
#endif
