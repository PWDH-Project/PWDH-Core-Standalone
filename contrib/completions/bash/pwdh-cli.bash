# bash programmable completion for pwdh-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $pwdh-cli for RPC
_pwdh_rpc() {
...
_pwdh_cli() {
    ...
    # save and use original argument to invoke pwdh-cli for -help, help and RPC
    # as pwdh-cli might not be in $PATH
    pwdh_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($pwdh_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_pwdh_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
