...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing pwdh-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='pwdh-cli', help='pwdh-cli command to use')
...
parser.add_argument('pwdh_cli_args', nargs='*', help='Arguments to pass on to pwdh-cli (default: -signet)')

args = parser.parse_args()

if args.pwdh_cli_args == []:
    args.pwdh_cli_args = ['-signet']


def pwdh_cli(rpc_command_and_params):
    argv = [args.cmd] + args.pwdh_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = pwdh_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = pwdh_cli(['getnewaddress', 'faucet', 'bech32'])
