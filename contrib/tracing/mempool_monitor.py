...
def main(pwdhd_path):
    pwdhd_with_usdts = USDT(path=str(pwdhd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    pwdhd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    pwdhd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    pwdhd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    pwdhd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[pwdhd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/pwdhd")
        exit(1)
