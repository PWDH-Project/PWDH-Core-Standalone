...
To build Bitcoin Core without Qt, unload or disable the `pwdh-qt`, `libpwdh_qt` and `test_pwdh-qt` projects.
...
```cmd
msbuild build_msvc\pwdh.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/pwdh.sln` file in Visual Studio.
...
To check if `pwdhd` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/pwdhd.exe
```
